...
function decode(text: string, max = Infinity): Uint8Array<ArrayBuffer> {
...
export function encryptedData(value: unknown): EncryptedData {
  ...
  if (!raw || raw.format !== 'terminal-reader-encrypted' || raw.version !== 1 || raw.kdf !== 'PBKDF2-SHA256'
    ...
    || decode(raw.salt, 16).length !== 16 || decode(raw.iv, 12).length !== 12 || typeof raw.ciphertext !== 'string') throw new Error('不支持或无效的加密文件');
  return raw;
}
...
export async function unseal(raw: EncryptedData, source: EncryptionKey): Promise<unknown> {
  ...
  try {
    const plain = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: decode(envelope.iv, 12) }, source.key, decode(envelope.ciphertext));
